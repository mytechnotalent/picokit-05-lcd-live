...
static void assert_lamps(int red, int yellow, int green) {
    TEST_ASSERT_EQUAL_INT(red, mock_gpio_get(PICOKIT_05_LCD_LIVE_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(PICOKIT_05_LCD_LIVE_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(PICOKIT_05_LCD_LIVE_GREEN_LED_PIN));
}
...
static void test_status_led_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_05_LCD_LIVE_RED_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_05_LCD_LIVE_YELLOW_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_05_LCD_LIVE_GREEN_LED_PIN]);
}
 ...
 */
static const uint8_t s_env_pt[] = "{\"n\":1,\"s\":0,\"b\":2}";
 ...
 */
static const char s_env_known_hex[] = "000102030405060708090a0b0c0d0e0f1011121314151617e5e0615daae3a18c40661cfee770caca717555a2a0057c16effc7cda52337905c66721";
...
/**
 * @brief Verify the 1602 LCD initialization writes the setup sequence.
 *
 * @param void No parameters.
 * @return void
 */
static void test_display_init(void) {
    reset_pc();
    TEST_ASSERT_TRUE(display_init(i2c1, PICOKIT_05_LCD_LIVE_LCD_ADDR));
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}

/**
 * @brief Verify the LCD initialization fails when the bus write fails.
 *
 * @param void No parameters.
 * @return void
 */
static void test_display_init_fails(void) {
    reset_pc();
    mock_i2c_set_write_fail(true);
    TEST_ASSERT_FALSE(display_init(i2c1, PICOKIT_05_LCD_LIVE_LCD_ADDR));
}

/**
 * @brief Verify both LCD rows are pushed over the I2C bus.
 *
 * @param void No parameters.
 * @return void
 */
static void test_display_show_lines(void) {
    reset_pc();
    display_init(i2c1, PICOKIT_05_LCD_LIVE_LCD_ADDR);
    mock_i2c_reset();
    display_show_lines(i2c1, PICOKIT_05_LCD_LIVE_LCD_ADDR, "HELLO", "WORLD");
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}

/**
 * @brief Run the 1602 LCD driver tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_display_tests(void) {
    ...
    RUN_TEST(test_display_show_lines);
}
